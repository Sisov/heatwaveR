...
#' out$event[1:5, 1:10]
#'
detect_event_cpp <- function(data,
                         x = t,
                         y = temp,
                         seasClim = seas,
                         threshClim = thresh,
  ...
  proto_1 <- cpp_proto_event(t_series, t_series$threshCriterion,
  ...
  proto_2 <- cpp_proto_event(t_series, t_series$durationCriterion,
  ...
  if (joinAcrossGaps) {
    ...
    for (i in 1:nrow(proto_2)) {
      t_series$event[proto_2$index_start[i]:proto_2$index_end[i]] <-
        as.numeric(rep(TRUE, length = proto_2$duration[i]))
    }
  } else {
    t_series$event <- t_series$durationCriterion
  }

  proto_3 <- cpp_proto_event(t_series, t_series$event,
                         minDuration = minDuration, maxGap = maxGap)
  ...
  events <- t_series
  events$row_index <- 1:nrow(t_series)
  events$mhw_rel_seas <- events$ts_y - events$ts_seas
  events$mhw_rel_thresh <- events$ts_y - events$ts_thresh
  ...
  events <- events[,{
    ...
    list(index_start = index_start, index_peak = index_peak,
         index_end = index_end, duration = duration,
         date_start = date_start, date_peak = date_peak,
         date_end = date_end, intensity_mean = intensity_mean,
         intensity_max = intensity_max, intensity_var = intensity_var,
